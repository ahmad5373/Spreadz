    ...
    return (
        <div className="bg-gray-100 lg:bg-white mt-4">
            ...
            <Container className='px-4 lg:px-10 xl1:px-20'>
                <div className="flex flex-col lg:flex-row justify-between gap-12 lg:gap-20 py-6 ">
                    {plans.map((plan, index) => (
                        <div
                            key={index}
                            className={`lg:relative lg:w-[362.67px]  py-8 px-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow ${plan.backgroundColor} ${plan.textColor}`}
                        >
                            {plan.title === "Pro" && (
                            ...
                            <div className="flex justify-between">
                                <h2 className="base-font-heading xl1:text-3xl lg:text-xl leading-[41.3px] mb-4 ">{plan.title}</h2>
                                <div className="flex justify-center items-center space-x-4 mb-4">
                                    <span className="base-font-heading text-sm">Yearly</span>
                                    <div
                                        className={`w-8 h-4 rounded-full cursor-pointer relative transition-colors duration-300 ${plan.title === "Pro" ? "bg-white" : "bg-gray-200"
                                            }`}
                                        onClick={() => toggleBilling(index)}
                                    >
                                        <div
                                            className={`absolute top-0.7 left-0.4 w-4 h-4 ${plan.title === "Pro" ? "bg-gray-300" : "bg-gray-300"} rounded-full shadow-md transition-all duration-300 ${billingCycles[index] === "yearly" ? "translate-x-5" : "translate-x-0.6"
                                                }`}
                                        ></div>
                                    </div>
                                </div>
                                       ...
                                       ${plan.title === "Pro"
                                        ...
                                        >
                                            {proFeatures.includes(feature) || excellenceFeatures.includes(feature) ? (
                                                <img src={PlusIcon} alt="PlusIcon Icon" className="h-4 w-4"></img>
                                            ) : (
                                                <img src={TickIcon} alt="TickIcon Icon" className="h-4 w-4"></img>
                                            )}
                                        </div>
